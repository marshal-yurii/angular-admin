  ...
  return new Observable<boolean>(subscriber => {
    ...
    triggers.forEach((trigger: Trigger) => {
      if (trigger.open === trigger.close) {
        ...
        listeners.push(
          renderer.listen(nativeElement, trigger.open as string, openFn),
          renderer.listen(nativeElement, trigger.close as string, closeFn));
      }
    });
...
export const triggerDelay = (openDelay: number, closeDelay: number, isOpenedFn: () => boolean) => {
  return (input$: Observable<boolean>) => {
    let pending: { open: boolean } | null = null;
    const filteredInput$ = input$.pipe(
      map(open => ({open})), filter((event: any) => {
